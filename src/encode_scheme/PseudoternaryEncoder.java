...
/**
 * PseudoternaryEncoder: Implements Pseudoternary encoding.
 * 
 * Pseudoternary encoding is a line encoding technique where each bit is
 * represented by the presence or absence of a transition. It uses three voltage
 * levels where a logic high is represented by no transition, and a logic low is
 * represented by alternating positive and negative voltage transitions.
 * Specifically, for a '0' bit, the signal transitions between positive and
 * negative levels, while for a '1' bit, there is no transition and the signal
 * remains at zero voltage.
 * 
 * Extends Encoder to draw Pseudoternary encoded signal.
 * 
 * Author: Sourashis Das
 */
public class PseudoternaryEncoder extends Encoder {

    // Constructors
	public PseudoternaryEncoder() {
		super("Pseudoternary",0); // Default constructor with no scale
	}
	
	public PseudoternaryEncoder(int scale) {
		super("Pseudoternary",scale); // Constructor with scale
	}
